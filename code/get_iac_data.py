...
def update_dataset_campinas() ->None:
    ...
    """
    #Campinas
    df = pl.read_parquet("./data/campinas.parquet")
    #df = pl.from_pandas(get_iac_table(url='https://clima.iac.sp.gov.br/index.php/main/detalhar/3', lat=-47.072878, lon=-22.867442))
    ...
    with open('./data/log.csv', 'a') as w:
        ...
        w.close()
    df.write_parquet("./data/campinas.parquet")
    
def update_dataset_piracicaba() ->None:
    """This function get new data of temperature, precipitation and relative humidity
    """
    #Campinas
    df = pl.read_parquet("./data/piracicaba.parquet")
    ...
    with open('./data/log.csv', 'a') as w:
        w.write(f"\n insert iac data at dataset | {datetime.today()}")
        w.close()
    df.write_parquet("./data/piracicaba.parquet")
